            ...
            onChange={async e => {
              ...
              image.onload = async () => {
                ...
                const imageBlob = (() => {
                  ...
                  const mimeString = newFileUrl.split(',')[0].split(':')[1].split(';')[0]
                ...
                const newFile: any = new File([imageBlob], `${session?.user_id}.webp`, { type: imageBlob.type })
                let [data, error] = await upload(newFile, 'profile_images', session?.user_id);
                [data, error] = await resolver(axios.patch(`/api/users/profile-data/update-profile-image?user_id=${data.user_id}&image_url=${data.image_url}`))
        ...
        <article className={styles.user_info}>
          ...
          <section>
            <Image
              ...
              width={144}
              height={144}
              src={profile_data.user_id === session?.user_id ? session?.image_url : profile_data.image_url}
            />
            <div>
